###########################################################
# Wyznaczenie wartoœci referencyjnych na bazie ROC = Receiver Operating Characteristic
###########################################################
#install.packages("pROC")
library(pROC)
# Funkcja w³asna w której obliczane s¹ Swoistoœæ i Czu³oœæ i przedzia³y ufnoœci
...
#----- Znalezienie zale¿noœci Swoistoœci i Czu³oœci
  jROC<-roc(Incydent~LVEF,data=Dane, percent=TRUE) 
#----- Znalezienie wartoœci progu i pozosta³ych
  jW<-coords(jROC, 'best', 
...
Wynik <- rbind(
 paste(c('Czu³oœæ:',CzuloscWynik,' (',CzuloscCI[1],'%',' -',CzuloscCI[2],'%',')'),sep='',collapse=''),
 paste(c('Swoistoœæ:', SwoistoscWynik,' (',SwoistoscCI[1],'%',' -',SwoistoscCI[2],'%',')'),sep='',collapse=''),
...
names(WynikTab) <- c('Czu³oœæ','Swoistoœæ','Predykcja dodatnia','Predykcja ujemna','n11','n12','n21','n22')
WynikTab
}
...
Wynik<-Wydruk(c(
'Czu³oœæ:',           ' p',ZnakMniejszosci(Czulosc),Czulosc,'; ',
'Swoistoœæ:',         ' p',ZnakMniejszosci(Swoistosc) ,Swoistosc,';\n', 
...
ogó³ wartoœci¹ uwa¿an¹ za w³asciw¹ dla osoby zdrowej jest $\\m{LVEF} > 30\\%.$
W naszym przypadku s¹ to pacjenci chorzy i wartoœæ ta jako diagnostyczna do 
decyzji o preferencji do wszczepienia rozrusznika mo¿e byæ ni¿sza.
\n\n Wartoœæ progow¹ (diagnostyczn¹) znajdujemy w wyniku procedury sekwencyjnej:
\n\n Ka¿da wartoœæ $\\m{LVEF} = n\\%$ dzieli pacjentów na dwie grupy, jedn¹ 
gdy $\\m{LVEF} \\leq n\\%$ (bardziej chorzy) i drug¹ o 
gdy $\\m{LVEF} > n\\%.$
Dla ka¿dego podzia³u znajdujemy wartoœæ swoistoœci i ufnoœci.
Wartoœæ progowa (diagnostyczna), to jest taka wartoœæ dla której suma 
swoistoœci i ufnoœci jest najwiêksza. \n\n
W rozwi¹zaniu zadania nale¿y podaæ
\\begin{enumerate}
\\item Wykres czu³oœci(sensitivity) w zale¿noœci od swoistoœci(specificity),
\\item ZnaleŸæ wartoœæ progow¹ (treshold),
\\item Dla znalezionej wartoœci progowej znaleŸæ 
czu³oœæ (sensitivity), swoistoœæ(specificity), 
predykcjê dodatni¹ (npv), predykcjê ujemn¹ (ppv),
\\item ZnaleŸæ 95\\% przedzia³y ufnoœci Wilsona dla 
czu³oœci, swoistoœci, predykcji dodatniej i predykcji ujemnej 
wyznaczonej przez wrtoœæ progow¹.
\\end{enumerate}')
